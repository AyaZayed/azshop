    ...
    <>
      {reviews.length > 0 && (
        <section className="p-6 py-16 flex flex-col items-center justify-center">
          <h1 className="capitalize mb-16">What our customers say</h1>
          <div
            className={`grid grid-cols-1 md:grid-cols-${reviews.length} gap-4 md:p-10`}>
            {reviews.map((review) => (
              <div
                key={review.id}
                className="p-6 border border-sf_sedcondary flex flex-col transition-transform duration-500 ease-in-out md:hover:-translate-y-6 justify-between">
